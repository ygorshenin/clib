...
void RadixSort(size_t n, const size_t* keys, size_t max_value, const Values& values, size_t* result_keys) {
...
size_t RestoreIndex(size_t n0, size_t pos) { return InLeftHalf(n0, pos) ? pos * 3 + 1 : (pos - n0) * 3 + 2; }
...
struct Slice {
  Slice(const Container& c, size_t n, size_t offset) : c_(c), n_(n), offset_(offset) {}
...
namespace algo {
void Skew(size_t n, const uint8_t* s, size_t* pos) { RawSkew(n, 0xFF /* max_value */, SkewWrapper(n, s), pos); }
...
void ManberMyers(size_t n, const uint8_t* s, size_t* pos) {
  ...
  const size_t max_class = std::max(n, static_cast<size_t>(std::numeric_limits<uint8_t>::max()) + 1);
  ...
  for (size_t h = 1; h < n; h <<= 1) {
    ...
    for (size_t i = 1; i < n; ++i) {
      ...
      cur_class += classes[cp] != classes[pp] || classes[(cp + h) % n] != classes[(pp + h) % n];
