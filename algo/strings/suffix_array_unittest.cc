...
namespace {
static_assert(sizeof(uint8_t) == sizeof(char), "sizes of char and uint8_t should be equal");

TEST(Skew, Smoke) { Skew(0, nullptr, nullptr); }
...
TEST(ManberMyers, Empty) { ManberMyers(0, nullptr, nullptr); }
...
TEST(ManberMyers, LCP) {
  ...
  Kasai(n, reinterpret_cast<const uint8_t*>(s), pos.data(), rank.data(), lcp.data());
  ASSERT_EQ(0, lcp[0]);
  ASSERT_EQ(2, lcp[1]);
  ASSERT_EQ(0, lcp[2]);
  ASSERT_EQ(1, lcp[3]);
